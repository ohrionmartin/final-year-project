...
/**
 * Eclinic_portal View class for the Antenatal_cares
 */
class Eclinic_portalViewAntenatal_cares extends JViewLegacy
{
	/**
	...
	function display($tpl = null)
	{
		if ($this->getLayout() !== 'modal')
		{
			...
			Eclinic_portalHelper::addSubmenu('antenatal_cares');
		}
		...
		$this->canDo = Eclinic_portalHelper::getActions('antenatal_care');
	...
	{
		JToolBarHelper::title(JText::_('COM_ECLINIC_PORTAL_ANTENATAL_CARES'), 'flash');
		JHtmlSidebar::setAction('index.php?option=com_eclinic_portal&view=antenatal_cares');
		...
		if (Eclinic_portalHelper::checkArray($this->items))
		{
			...
			{
				JToolBarHelper::custom('antenatal_cares.exportData', 'download', '', 'COM_ECLINIC_PORTAL_EXPORT_DATA', true);
			}
		}

		if ($this->canDo->get('core.import') && $this->canDo->get('antenatal_care.import'))
		{
			JToolBarHelper::custom('antenatal_cares.importData', 'upload', '', 'COM_ECLINIC_PORTAL_IMPORT_DATA', false);
		}

		// set help url for this view if found
		$help_url = Eclinic_portalHelper::getHelpUrl('antenatal_cares');
		...
		if (Eclinic_portalHelper::checkArray($this->patientOptions))
		{
			...
			JHtmlSidebar::addFilter(
				'- Select '.JText::_('COM_ECLINIC_PORTAL_ANTENATAL_CARE_PATIENT_LABEL').' -',
				'filter_patient',
			...
			if ($this->canBatch && $this->canCreate && $this->canEdit)
			{
				...
				JHtmlBatch_::addListSelection(
					'- Keep Original '.JText::_('COM_ECLINIC_PORTAL_ANTENATAL_CARE_PATIENT_LABEL').' -',
					'batch[patient]',
		...
		// [Interpretation 17529] Set Foetal Lie Name Selection
		$this->foetal_lieNameOptions = JFormHelper::loadFieldType('Foetallie')->options;
		// [Interpretation 17535] We do some sanitation for Foetal Lie Name filter
		if (Eclinic_portalHelper::checkArray($this->foetal_lieNameOptions) &&
			isset($this->foetal_lieNameOptions[0]->value) &&
			!Eclinic_portalHelper::checkString($this->foetal_lieNameOptions[0]->value))
		{
			unset($this->foetal_lieNameOptions[0]);
		}
		// [Interpretation 17551] Only load Foetal Lie Name filter if it has values
		if (Eclinic_portalHelper::checkArray($this->foetal_lieNameOptions))
		{
			// [Interpretation 17559] Foetal Lie Name Filter
			JHtmlSidebar::addFilter(
				'- Select '.JText::_('COM_ECLINIC_PORTAL_ANTENATAL_CARE_FOETAL_LIE_LABEL').' -',
				'filter_foetal_lie',
				JHtml::_('select.options', $this->foetal_lieNameOptions, 'value', 'text', $this->state->get('filter.foetal_lie'))
			);

			if ($this->canBatch && $this->canCreate && $this->canEdit)
			{
				// [Interpretation 17576] Foetal Lie Name Batch Selection
				JHtmlBatch_::addListSelection(
					'- Keep Original '.JText::_('COM_ECLINIC_PORTAL_ANTENATAL_CARE_FOETAL_LIE_LABEL').' -',
					'batch[foetal_lie]',
	...
	protected function setDocument()
	{
		...
		$this->document->setTitle(JText::_('COM_ECLINIC_PORTAL_ANTENATAL_CARES'));
		$this->document->addStyleSheet(JURI::root() . "administrator/components/com_eclinic_portal/assets/css/antenatal_cares.css", (Eclinic_portalHelper::jVersion()->isCompatible('3.8.0')) ? array('version' => 'auto') : 'text/css');
	}
	...
	protected function getSortFields()
	{
		return array(
			...
			'a.patient' => JText::_('COM_ECLINIC_PORTAL_ANTENATAL_CARE_PATIENT_LABEL'),

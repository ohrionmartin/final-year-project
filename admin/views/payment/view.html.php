 ...
 * Payment View class
 */
class Eclinic_portalViewPayment extends JViewLegacy
{
	/**
	...
	public function display($tpl = null)
	{
		...
		$this->canDo = Eclinic_portalHelper::getActions('payment', $this->item);
		// get input
	...
	protected function addToolBar()
	{
		...
		JToolbarHelper::title( JText::_($isNew ? 'COM_ECLINIC_PORTAL_PAYMENT_NEW' : 'COM_ECLINIC_PORTAL_PAYMENT_EDIT'), 'pencil-2 article-add');
		...
		else
		{
			...
			else
			{
				...
				{
					JToolbarHelper::versions('com_eclinic_portal.payment', $this->item->id);
				}
				if ($this->canDo->get('core.create'))
				{
		...
		$help_url = Eclinic_portalHelper::getHelpUrl('payment');
	...
	protected function setDocument()
	{
		...
		$this->document->setTitle(JText::_($isNew ? 'COM_ECLINIC_PORTAL_PAYMENT_NEW' : 'COM_ECLINIC_PORTAL_PAYMENT_EDIT'));
		$this->document->addStyleSheet(JURI::root() . "administrator/components/com_eclinic_portal/assets/css/payment.css", (Eclinic_portalHelper::jVersion()->isCompatible('3.8.0')) ? array('version' => 'auto') : 'text/css');
		...
		$this->document->addScript(JURI::root() . "administrator/components/com_eclinic_portal/views/payment/submitbutton.js", (Eclinic_portalHelper::jVersion()->isCompatible('3.8.0')) ? array('version' => 'auto') : 'text/javascript'); 

 ...
 * Site View class
 */
class Eclinic_portalViewSite extends JViewLegacy
{
	/**
	...
	public function display($tpl = null)
	{
		...
		$this->canDo = Eclinic_portalHelper::getActions('site', $this->item);
		// get input
	...
	protected function addToolBar()
	{
		...
		JToolbarHelper::title( JText::_($isNew ? 'COM_ECLINIC_PORTAL_SITE_NEW' : 'COM_ECLINIC_PORTAL_SITE_EDIT'), 'pencil-2 article-add');
		...
		else
		{
			...
			else
			{
				...
				{
					JToolbarHelper::versions('com_eclinic_portal.site', $this->item->id);
				}
				if ($this->canDo->get('core.create'))
				{
		...
		$help_url = Eclinic_portalHelper::getHelpUrl('site');
	...
	protected function setDocument()
	{
		...
		$this->document->setTitle(JText::_($isNew ? 'COM_ECLINIC_PORTAL_SITE_NEW' : 'COM_ECLINIC_PORTAL_SITE_EDIT'));
		$this->document->addStyleSheet(JURI::root() . "administrator/components/com_eclinic_portal/assets/css/site.css", (Eclinic_portalHelper::jVersion()->isCompatible('3.8.0')) ? array('version' => 'auto') : 'text/css');
		...
		$this->document->addScript(JURI::root() . "administrator/components/com_eclinic_portal/views/site/submitbutton.js", (Eclinic_portalHelper::jVersion()->isCompatible('3.8.0')) ? array('version' => 'auto') : 'text/javascript'); 

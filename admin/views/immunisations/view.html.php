...
/**
 * Eclinic_portal View class for the Immunisations
 */
class Eclinic_portalViewImmunisations extends JViewLegacy
{
	/**
	...
	function display($tpl = null)
	{
		if ($this->getLayout() !== 'modal')
		{
			...
			Eclinic_portalHelper::addSubmenu('immunisations');
		}
	...
	{
		JToolBarHelper::title(JText::_('COM_ECLINIC_PORTAL_IMMUNISATIONS'), 'calendar-3');
		JHtmlSidebar::setAction('index.php?option=com_eclinic_portal&view=immunisations');
		...
		if (Eclinic_portalHelper::checkArray($this->items))
		{
			...
			{
				JToolBarHelper::custom('immunisations.exportData', 'download', '', 'COM_ECLINIC_PORTAL_EXPORT_DATA', true);
			}
		}

		if ($this->canDo->get('core.import') && $this->canDo->get('immunisation.import'))
		{
			JToolBarHelper::custom('immunisations.importData', 'upload', '', 'COM_ECLINIC_PORTAL_IMPORT_DATA', false);
		}

		// set help url for this view if found
		$help_url = Eclinic_portalHelper::getHelpUrl('immunisations');
		...
		if (Eclinic_portalHelper::checkArray($this->patientOptions))
		{
			...
			JHtmlSidebar::addFilter(
				'- Select '.JText::_('COM_ECLINIC_PORTAL_IMMUNISATION_PATIENT_LABEL').' -',
				'filter_patient',
			...
			if ($this->canBatch && $this->canCreate && $this->canEdit)
			{
				...
				JHtmlBatch_::addListSelection(
					'- Keep Original '.JText::_('COM_ECLINIC_PORTAL_IMMUNISATION_PATIENT_LABEL').' -',
					'batch[patient]',
	...
	protected function setDocument()
	{
		...
		$this->document->setTitle(JText::_('COM_ECLINIC_PORTAL_IMMUNISATIONS'));
		$this->document->addStyleSheet(JURI::root() . "administrator/components/com_eclinic_portal/assets/css/immunisations.css", (Eclinic_portalHelper::jVersion()->isCompatible('3.8.0')) ? array('version' => 'auto') : 'text/css');
	}
	...
	protected function getSortFields()
	{
		return array(
			...
			'a.patient' => JText::_('COM_ECLINIC_PORTAL_IMMUNISATION_PATIENT_LABEL'),

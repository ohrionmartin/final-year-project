...
/**
 * Eclinic_portal View class for the Tests
 */
class Eclinic_portalViewTests extends JViewLegacy
{
	/**
	...
	function display($tpl = null)
	{
		if ($this->getLayout() !== 'modal')
		{
			...
			Eclinic_portalHelper::addSubmenu('tests');
		}
	...
	{
		JToolBarHelper::title(JText::_('COM_ECLINIC_PORTAL_TESTS'), 'chart');
		JHtmlSidebar::setAction('index.php?option=com_eclinic_portal&view=tests');
		...
		if (Eclinic_portalHelper::checkArray($this->items))
		{
			...
			{
				JToolBarHelper::custom('tests.exportData', 'download', '', 'COM_ECLINIC_PORTAL_EXPORT_DATA', true);
			}
		}

		if ($this->canDo->get('core.import') && $this->canDo->get('test.import'))
		{
			JToolBarHelper::custom('tests.importData', 'upload', '', 'COM_ECLINIC_PORTAL_IMPORT_DATA', false);
		}

		// set help url for this view if found
		$help_url = Eclinic_portalHelper::getHelpUrl('tests');
		...
		if (Eclinic_portalHelper::checkArray($this->patientOptions))
		{
			...
			JHtmlSidebar::addFilter(
				'- Select '.JText::_('COM_ECLINIC_PORTAL_TEST_PATIENT_LABEL').' -',
				'filter_patient',
			...
			if ($this->canBatch && $this->canCreate && $this->canEdit)
			{
				...
				JHtmlBatch_::addListSelection(
					'- Keep Original '.JText::_('COM_ECLINIC_PORTAL_TEST_PATIENT_LABEL').' -',
					'batch[patient]',
	...
	protected function setDocument()
	{
		...
		$this->document->setTitle(JText::_('COM_ECLINIC_PORTAL_TESTS'));
		$this->document->addStyleSheet(JURI::root() . "administrator/components/com_eclinic_portal/assets/css/tests.css", (Eclinic_portalHelper::jVersion()->isCompatible('3.8.0')) ? array('version' => 'auto') : 'text/css');
	}
	...
	protected function getSortFields()
	{
		return array(
			...
			'a.patient' => JText::_('COM_ECLINIC_PORTAL_TEST_PATIENT_LABEL'),

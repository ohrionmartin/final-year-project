 ...
 * Test View class
 */
class Eclinic_portalViewTest extends JViewLegacy
{
	/**
	...
	public function display($tpl = null)
	{
		...
		$this->canDo = Eclinic_portalHelper::getActions('test', $this->item);
		// get input
	...
	protected function addToolBar()
	{
		...
		JToolbarHelper::title( JText::_($isNew ? 'COM_ECLINIC_PORTAL_TEST_NEW' : 'COM_ECLINIC_PORTAL_TEST_EDIT'), 'pencil-2 article-add');
		...
		else
		{
			...
			else
			{
				...
				{
					JToolbarHelper::versions('com_eclinic_portal.test', $this->item->id);
				}
				if ($this->canDo->get('core.create'))
				{
		...
		$help_url = Eclinic_portalHelper::getHelpUrl('test');
	...
	protected function setDocument()
	{
		...
		$this->document->setTitle(JText::_($isNew ? 'COM_ECLINIC_PORTAL_TEST_NEW' : 'COM_ECLINIC_PORTAL_TEST_EDIT'));
		$this->document->addStyleSheet(JURI::root() . "administrator/components/com_eclinic_portal/assets/css/test.css", (Eclinic_portalHelper::jVersion()->isCompatible('3.8.0')) ? array('version' => 'auto') : 'text/css');
		...
		$this->document->addScript(JURI::root() . "administrator/components/com_eclinic_portal/views/test/submitbutton.js", (Eclinic_portalHelper::jVersion()->isCompatible('3.8.0')) ? array('version' => 'auto') : 'text/javascript'); 

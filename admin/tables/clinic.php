 ...
 * Clinics Table class
 */
class Eclinic_portalTableClinic extends JTable
{
	/**
	...
	{
		parent::__construct('#__eclinic_portal_clinic', 'id', $db);

		// [Interpretation 7717] Adding History Options
		JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_eclinic_portal.clinic'));
	}	
	...
	public function store($updateNulls = false)
	{
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(JText::_('COM_ECLINIC_PORTAL_CLINIC_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_eclinic_portal.clinic.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_eclinic_portal.clinic.'.(int) $this->$k;
	}

 ...
 */
class Eclinic_portalTableTuberculosis extends JTable
{
	/**
	...
	{
		parent::__construct('#__eclinic_portal_tuberculosis', 'id', $db);

		// [Interpretation 7717] Adding History Options
		JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_eclinic_portal.tuberculosis'));
	}	
	...
	public function store($updateNulls = false)
	{
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('tuberculosis', 'Eclinic_portalTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_ECLINIC_PORTAL_TUBERCULOSIS_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_eclinic_portal.tuberculosis.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_eclinic_portal.tuberculosis.'.(int) $this->$k;
	}

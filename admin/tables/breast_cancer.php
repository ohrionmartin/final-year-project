 ...
 */
class Eclinic_portalTableBreast_cancer extends JTable
{
	/**
	...
	{
		parent::__construct('#__eclinic_portal_breast_cancer', 'id', $db);

		// [Interpretation 7724] Adding History Options
		JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_eclinic_portal.breast_cancer'));
	}	
	...
	public function store($updateNulls = false)
	{
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('breast_cancer', 'Eclinic_portalTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_ECLINIC_PORTAL_BREAST_CANCER_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_eclinic_portal.breast_cancer.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_eclinic_portal.breast_cancer.'.(int) $this->$k;
	}

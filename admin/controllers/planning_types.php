 ...
 */
class Eclinic_portalControllerPlanning_types extends JControllerAdmin
{
	/**
	 ...
	 * @since  1.6
	 */
	protected $text_prefix = 'COM_ECLINIC_PORTAL_PLANNING_TYPES';
	 ...
	 * @since   1.6
	 */
	public function getModel($name = 'Planning_type', $prefix = 'Eclinic_portalModel', $config = array('ignore_request' => true))
	{
	...
	public function exportData()
	{
		...
		if ($user->authorise('planning_type.export', 'com_eclinic_portal') && $user->authorise('core.export', 'com_eclinic_portal'))
		{
			...
			if (Eclinic_portalHelper::checkArray($data))
			{
				...
				Eclinic_portalHelper::xls($data,'Planning_types_'.$date->format('jS_F_Y'),'Planning types exported ('.$date->format('jS F, Y').')','planning types');
			}
		}
		// [Interpretation 14792] Redirect to the list screen with error.
		$message = JText::_('COM_ECLINIC_PORTAL_EXPORT_FAILED');
		$this->setRedirect(JRoute::_('index.php?option=com_eclinic_portal&view=planning_types', false), $message, 'error');
		return;
	}
	...
	public function importData()
	{
		...
		$user = JFactory::getUser();
		if ($user->authorise('planning_type.import', 'com_eclinic_portal') && $user->authorise('core.import', 'com_eclinic_portal'))
		{
			...
			if (Eclinic_portalHelper::checkObject($headers))
			{
				...
				$message = JText::_('COM_ECLINIC_PORTAL_IMPORT_SELECT_FILE_FOR_PLANNING_TYPES');

 ...
 */
class Ehealth_portalControllerGroup_health_education_topics extends JControllerAdmin
{
	/**
	 ...
	 * @since  1.6
	 */
	protected $text_prefix = 'COM_EHEALTH_PORTAL_GROUP_HEALTH_EDUCATION_TOPICS';
	 ...
	 * @since   1.6
	 */
	public function getModel($name = 'Group_health_education_topic', $prefix = 'Ehealth_portalModel', $config = array('ignore_request' => true))
	{
	...
	public function exportData()
	{
		...
		if ($user->authorise('group_health_education_topic.export', 'com_ehealth_portal') && $user->authorise('core.export', 'com_ehealth_portal'))
		{
			...
			if (Ehealth_portalHelper::checkArray($data))
			{
				...
				Ehealth_portalHelper::xls($data,'Group_health_education_topics_'.$date->format('jS_F_Y'),'Group health education topics exported ('.$date->format('jS F, Y').')','group health education topics');
			}
		}
		// [Interpretation 14792] Redirect to the list screen with error.
		$message = JText::_('COM_EHEALTH_PORTAL_EXPORT_FAILED');
		$this->setRedirect(JRoute::_('index.php?option=com_ehealth_portal&view=group_health_education_topics', false), $message, 'error');
		return;
	}
	...
	public function importData()
	{
		...
		$user = JFactory::getUser();
		if ($user->authorise('group_health_education_topic.import', 'com_ehealth_portal') && $user->authorise('core.import', 'com_ehealth_portal'))
		{
			...
			if (Ehealth_portalHelper::checkObject($headers))
			{
				...
				$message = JText::_('COM_EHEALTH_PORTAL_IMPORT_SELECT_FILE_FOR_GROUP_HEALTH_EDUCATION_TOPICS');

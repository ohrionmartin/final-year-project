 ...
 */
class Ehealth_portalControllerIndividual_health_educations extends JControllerAdmin
{
	/**
	 ...
	 * @since  1.6
	 */
	protected $text_prefix = 'COM_EHEALTH_PORTAL_INDIVIDUAL_HEALTH_EDUCATIONS';
	 ...
	 * @since   1.6
	 */
	public function getModel($name = 'Individual_health_education', $prefix = 'Ehealth_portalModel', $config = array('ignore_request' => true))
	{
	...
	public function exportData()
	{
		...
		if ($user->authorise('individual_health_education.export', 'com_ehealth_portal') && $user->authorise('core.export', 'com_ehealth_portal'))
		{
			...
			if (Ehealth_portalHelper::checkArray($data))
			{
				...
				Ehealth_portalHelper::xls($data,'Individual_health_educations_'.$date->format('jS_F_Y'),'Individual health educations exported ('.$date->format('jS F, Y').')','individual health educations');
			}
		}
		// [Interpretation 14792] Redirect to the list screen with error.
		$message = JText::_('COM_EHEALTH_PORTAL_EXPORT_FAILED');
		$this->setRedirect(JRoute::_('index.php?option=com_ehealth_portal&view=individual_health_educations', false), $message, 'error');
		return;
	}
	...
	public function importData()
	{
		...
		$user = JFactory::getUser();
		if ($user->authorise('individual_health_education.import', 'com_ehealth_portal') && $user->authorise('core.import', 'com_ehealth_portal'))
		{
			...
			if (Ehealth_portalHelper::checkObject($headers))
			{
				...
				$message = JText::_('COM_EHEALTH_PORTAL_IMPORT_SELECT_FILE_FOR_INDIVIDUAL_HEALTH_EDUCATIONS');

 ...
 * Sites Controller
 */
class Eclinic_portalControllerSites extends JControllerAdmin
{
	/**
	 ...
	 * @since  1.6
	 */
	protected $text_prefix = 'COM_ECLINIC_PORTAL_SITES';
	 ...
	 * @since   1.6
	 */
	public function getModel($name = 'Site', $prefix = 'Eclinic_portalModel', $config = array('ignore_request' => true))
	{
	...
	public function exportData()
	{
		...
		if ($user->authorise('site.export', 'com_eclinic_portal') && $user->authorise('core.export', 'com_eclinic_portal'))
		{
			...
			if (Eclinic_portalHelper::checkArray($data))
			{
				...
				Eclinic_portalHelper::xls($data,'Sites_'.$date->format('jS_F_Y'),'Sites exported ('.$date->format('jS F, Y').')','sites');
			}
		}
		// [Interpretation 14792] Redirect to the list screen with error.
		$message = JText::_('COM_ECLINIC_PORTAL_EXPORT_FAILED');
		$this->setRedirect(JRoute::_('index.php?option=com_eclinic_portal&view=sites', false), $message, 'error');
		return;
	}
	...
	public function importData()
	{
		...
		$user = JFactory::getUser();
		if ($user->authorise('site.import', 'com_eclinic_portal') && $user->authorise('core.import', 'com_eclinic_portal'))
		{
			...
			if (Eclinic_portalHelper::checkObject($headers))
			{
				...
				$message = JText::_('COM_ECLINIC_PORTAL_IMPORT_SELECT_FILE_FOR_SITES');

...
/**
 * Group_health_educations Model
 */
class Eclinic_portalModelGroup_health_educations extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$group_health_edu = $this->getUserStateFromRequest($this->context . '.filter.group_health_edu', 'filter_group_health_edu');
		if ($formSubmited)
		{
			$group_health_edu = $app->input->post->get('group_health_edu');
			$this->setState('filter.group_health_edu', $group_health_edu);
		}
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__eclinic_portal_group_health_education', 'a'));

		// [Interpretation 15852] From the eclinic_portal_group_health_education_topic table.
		$query->select($db->quoteName('g.name','group_health_edu_name'));
		$query->join('LEFT', $db->quoteName('#__eclinic_portal_group_health_education_topic', 'g') . ' ON (' . $db->quoteName('a.group_health_edu') . ' = ' . $db->quoteName('g.id') . ')');
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('(a.patient LIKE '.$search.' OR a.group_health_edu LIKE '.$search.' OR g.name LIKE '.$search.')');
			}
		}
	...
	public function getExportData($pks, $user = null)
	{
		...
		if (($pks_size = Eclinic_portalHelper::checkArray($pks)) !== false || 'bulk' === $pks)
		{
			...
			// [Interpretation 15018] From the eclinic_portal_group_health_education table
			$query->from($db->quoteName('#__eclinic_portal_group_health_education', 'a'));
	...
	*/
	public function getExImPortHeaders()
	{
		...
		$db = JFactory::getDbo();
		// get the columns
		$columns = $db->getTableColumns("#__eclinic_portal_group_health_education");
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':' . $this->getState('filter.group_health_edu');
	...
	protected function checkInNow()
	{
		...
		if ($time)
		{
			...
			if ($db->getNumRows())
			{
				...
				$query->update($db->quoteName('#__eclinic_portal_group_health_education'))->set($fields)->where($conditions); 

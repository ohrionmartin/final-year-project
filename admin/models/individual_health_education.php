...
/-------------------------------------------------------------------------------------------------------/
	...
	@subpackage		individual_health_education.php
...
/**
 * Eclinic_portal Individual_health_education Model
 */
class Eclinic_portalModelIndividual_health_education extends JModelAdmin
{
	/**
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_eclinic_portal.individual_health_education';
	 ...
	 * @since   1.6
	 */
	public function getTable($type = 'individual_health_education', $prefix = 'Eclinic_portalTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_eclinic_portal.individual_health_education');
			}
		}
	...
	public function getForm($data = array(), $loadData = true, $options = array('control' => 'jform'))
	{
		...
		$form = $this->loadForm('com_eclinic_portal.individual_health_education', 'individual_health_education', $options, $clear, $xpath);
		...
		if ($id != 0 && (!$user->authorise('core.edit.state', 'com_eclinic_portal.individual_health_education.' . (int) $id))
		...
		// [Interpretation 19813] Modify the form based on Edit Individual Health Edu access controls.
		if ($id != 0 && (!$user->authorise('individual_health_education.edit.individual_health_edu', 'com_eclinic_portal.individual_health_education.' . (int) $id))
			|| ($id == 0 && !$user->authorise('individual_health_education.edit.individual_health_edu', 'com_eclinic_portal')))
		{
			// [Interpretation 19824] Disable fields for display.
			$form->setFieldAttribute('individual_health_edu', 'disabled', 'true');
			// [Interpretation 19828] Disable fields for display.
			$form->setFieldAttribute('individual_health_edu', 'readonly', 'true');
			// [Interpretation 19842] If there is no value continue.
			if (!$form->getValue('individual_health_edu'))
			{
				// [Interpretation 19847] Disable fields while saving.
				$form->setFieldAttribute('individual_health_edu', 'filter', 'unset');
				// [Interpretation 19851] Disable fields while saving.
				$form->setFieldAttribute('individual_health_edu', 'required', 'false');
			}
		}
	...
	{
		return 'administrator/components/com_eclinic_portal/models/forms/individual_health_education.js';
	}
	...
	protected function canDelete($record)
	{
		if (!empty($record->id))
		{
			...
			return $user->authorise('core.delete', 'com_eclinic_portal.individual_health_education.' . (int) $record->id);
		}
		return false;
	}
	...
	protected function canEditState($record)
	{
		...
		if ($recordId)
		{
			...
			$permission = $user->authorise('core.edit.state', 'com_eclinic_portal.individual_health_education.' . (int) $recordId);
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		return JFactory::getUser()->authorise('core.edit', 'com_eclinic_portal.individual_health_education.'. ((int) isset($data[$key]) ? $data[$key] : 0)) or parent::allowEdit($data, $key);
	}
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					->select('MAX(ordering)')
					->from($db->quoteName('#__eclinic_portal_individual_health_education'));
				$db->setQuery($query);
				$max = $db->loadResult();
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_eclinic_portal.edit.individual_health_education.data', array());

		if (empty($data))
		{
			$data = $this->getItem();
			// run the perprocess of the data
			$this->preprocessData('com_eclinic_portal.individual_health_education', $data);
		}
	...
	public function batch($commands, $pks, $contexts)
	{
		...
		$this->canDo			= Eclinic_portalHelper::getActions('individual_health_education');
		$this->batchSet			= true;

...
/**
 * Ehealth_portal Group_health_education Model
 */
class Ehealth_portalModelGroup_health_education extends JModelAdmin
{
	/**
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_ehealth_portal.group_health_education';
	 ...
	 * @since   1.6
	 */
	public function getTable($type = 'group_health_education', $prefix = 'Ehealth_portalTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_ehealth_portal.group_health_education');
			}
		}
	...
	public function getForm($data = array(), $loadData = true, $options = array('control' => 'jform'))
	{
		...
		$form = $this->loadForm('com_ehealth_portal.group_health_education', 'group_health_education', $options, $clear, $xpath);
		...
		if ($id != 0 && (!$user->authorise('core.edit.state', 'com_ehealth_portal.group_health_education.' . (int) $id))
		...
		// [Interpretation 18808] Modify the form based on Edit Group Health Edu access controls.
		if ($id != 0 && (!$user->authorise('group_health_education.edit.group_health_edu', 'com_ehealth_portal.group_health_education.' . (int) $id))
			|| ($id == 0 && !$user->authorise('group_health_education.edit.group_health_edu', 'com_ehealth_portal')))
		{
	...
	{
		return 'administrator/components/com_ehealth_portal/models/forms/group_health_education.js';
	}
	...
	protected function canDelete($record)
	{
		if (!empty($record->id))
		{
			...
			return $user->authorise('core.delete', 'com_ehealth_portal.group_health_education.' . (int) $record->id);
		}
		return false;
	}
	...
	protected function canEditState($record)
	{
		...
		if ($recordId)
		{
			...
			$permission = $user->authorise('core.edit.state', 'com_ehealth_portal.group_health_education.' . (int) $recordId);
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		return JFactory::getUser()->authorise('core.edit', 'com_ehealth_portal.group_health_education.'. ((int) isset($data[$key]) ? $data[$key] : 0)) or parent::allowEdit($data, $key);
	}
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					->select('MAX(ordering)')
					->from($db->quoteName('#__ehealth_portal_group_health_education'));
				$db->setQuery($query);
				$max = $db->loadResult();
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_ehealth_portal.edit.group_health_education.data', array());

		if (empty($data))
		{
			$data = $this->getItem();
			// run the perprocess of the data
			$this->preprocessData('com_ehealth_portal.group_health_education', $data);
		}
	...
	public function batch($commands, $pks, $contexts)
	{
		...
		$this->canDo			= Ehealth_portalHelper::getActions('group_health_education');
		$this->batchSet			= true;

 ...
 */
class Eclinic_portalModelPayment_amounts extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__eclinic_portal_payment_amount', 'a'));
	...
	public function getExportData($pks, $user = null)
	{
		...
		if (($pks_size = Eclinic_portalHelper::checkArray($pks)) !== false || 'bulk' === $pks)
		{
			...
			// [Interpretation 14528] From the eclinic_portal_payment_amount table
			$query->from($db->quoteName('#__eclinic_portal_payment_amount', 'a'));
	...
	*/
	public function getExImPortHeaders()
	{
		...
		$db = JFactory::getDbo();
		// get the columns
		$columns = $db->getTableColumns("#__eclinic_portal_payment_amount");
	...
	protected function checkInNow()
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__eclinic_portal_payment_amount'));
			$db->setQuery($query);
			$db->execute();
			if ($db->getNumRows())
			{
				...
				$query->update($db->quoteName('#__eclinic_portal_payment_amount'))->set($fields)->where($conditions); 

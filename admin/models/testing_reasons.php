 ...
 */
class Eclinic_portalModelTesting_reasons extends JModelList
{
	...
	public function getExportData($pks, $user = null)
	{
		...
		if (($pks_size = Eclinic_portalHelper::checkArray($pks)) !== false || 'bulk' === $pks)
		{
			...
			// [Interpretation 15018] From the eclinic_portal_testing_reason table
			$query->from($db->quoteName('#__eclinic_portal_testing_reason', 'a'));
	...
	*/
	...
	protected function checkInNow()
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__eclinic_portal_testing_reason'));
			$db->setQuery($query);
			$db->execute();
			if ($db->getNumRows())
			{
				...
				$query->update($db->quoteName('#__eclinic_portal_testing_reason'))->set($fields)->where($conditions); 

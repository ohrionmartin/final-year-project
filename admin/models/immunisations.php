 ...
 * Immunisations Model
 */
class Eclinic_portalModelImmunisations extends JModelList
{
	...
	public function getItems()
	{
		...
		if (Eclinic_portalHelper::checkArray($items))
		{
			...
			{
				// [Interpretation 21036] convert immunisation_up_to_date
	...
	{
		// [Interpretation 21076] Array of immunisation_up_to_date language strings
		if ($name === 'immunisation_up_to_date')
		{
			$immunisation_up_to_dateArray = array(
				0 => 'COM_ECLINIC_PORTAL_IMMUNISATION_YES',
				1 => 'COM_ECLINIC_PORTAL_IMMUNISATION_NO'
			);
			// [Interpretation 21113] Now check if value is found in this array
			if (isset($immunisation_up_to_dateArray[$value]) && Eclinic_portalHelper::checkString($immunisation_up_to_dateArray[$value]))
			{
	...
	public function getExportData($pks, $user = null)
	{
		...
		if (($pks_size = Eclinic_portalHelper::checkArray($pks)) !== false || 'bulk' === $pks)
		{
			...
			// [Interpretation 14528] From the eclinic_portal_immunisation table
			$query->from($db->quoteName('#__eclinic_portal_immunisation', 'a'));
	...
	*/
	public function getExImPortHeaders()
	{
		...
		$db = JFactory::getDbo();
		// get the columns
		$columns = $db->getTableColumns("#__eclinic_portal_immunisation");
	...
	protected function checkInNow()
	{
		...
		if ($time)
		{
			...
			if ($db->getNumRows())
			{
				...
				$query->update($db->quoteName('#__eclinic_portal_immunisation'))->set($fields)->where($conditions); 

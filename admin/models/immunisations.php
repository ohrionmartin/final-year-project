...
class Ehealth_portalModelImmunisations extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		// [Interpretation 15382] From the ehealth_portal_immunisation_type table.
		$query->select($db->quoteName('g.name','immunisation_type_name'));
		$query->join('LEFT', $db->quoteName('#__ehealth_portal_immunisation_type', 'g') . ' ON (' . $db->quoteName('a.immunisation_type') . ' = ' . $db->quoteName('g.id') . ')');

		// [Interpretation 15382] From the ehealth_portal_immunisation_vaccine_type table.
	...
	public function getExportData($pks, $user = null)
	{
		...
		if (($pks_size = Ehealth_portalHelper::checkArray($pks)) !== false || 'bulk' === $pks)
		{
			...
			// [Interpretation 14528] From the ehealth_portal_immunisation table
			$query->from($db->quoteName('#__ehealth_portal_immunisation', 'a'));

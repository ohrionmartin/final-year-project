...
/**
 * Eclinic_portal Immunisation Model
 */
class Eclinic_portalModelImmunisation extends JModelAdmin
{
	/**
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_eclinic_portal.immunisation';
	 ...
	 * @since   1.6
	 */
	public function getTable($type = 'immunisation', $prefix = 'Eclinic_portalTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			{
				// [Interpretation 7421] Convert the immunisation field to an array.
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_eclinic_portal.immunisation');
			}
		}
	...
	public function getForm($data = array(), $loadData = true, $options = array('control' => 'jform'))
	{
		...
		$form = $this->loadForm('com_eclinic_portal.immunisation', 'immunisation', $options, $clear, $xpath);
		...
		if ($id != 0 && (!$user->authorise('core.edit.state', 'com_eclinic_portal.immunisation.' . (int) $id))
		...
		// [Interpretation 19813] Modify the form based on Edit Patient access controls.
		if ($id != 0 && (!$user->authorise('immunisation.edit.patient', 'com_eclinic_portal.immunisation.' . (int) $id))
			|| ($id == 0 && !$user->authorise('immunisation.edit.patient', 'com_eclinic_portal')))
		{
	...
	{
		return 'administrator/components/com_eclinic_portal/models/forms/immunisation.js';
	}
	...
	protected function canDelete($record)
	{
		if (!empty($record->id))
		{
			...
			return $user->authorise('core.delete', 'com_eclinic_portal.immunisation.' . (int) $record->id);
		}
		return false;
	}
	...
	protected function canEditState($record)
	{
		...
		if ($recordId)
		{
			...
			$permission = $user->authorise('core.edit.state', 'com_eclinic_portal.immunisation.' . (int) $recordId);
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		return JFactory::getUser()->authorise('core.edit', 'com_eclinic_portal.immunisation.'. ((int) isset($data[$key]) ? $data[$key] : 0)) or parent::allowEdit($data, $key);
	}
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					->select('MAX(ordering)')
					->from($db->quoteName('#__eclinic_portal_immunisation'));
				$db->setQuery($query);
				$max = $db->loadResult();
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_eclinic_portal.edit.immunisation.data', array());

		if (empty($data))
		{
			$data = $this->getItem();
			// run the perprocess of the data
			$this->preprocessData('com_eclinic_portal.immunisation', $data);
		}
	...
	public function batch($commands, $pks, $contexts)
	{
		...
		$this->canDo			= Eclinic_portalHelper::getActions('immunisation');
		$this->batchSet			= true;
	...
	public function save($data)
	{
		...
		// [Interpretation 7521] Set the immunisation items to data.
		...
		{
			// [Interpretation 7571] Set the empty immunisation to data
			$data['immunisation'] = '';
		}

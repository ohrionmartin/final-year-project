 ...
 */
class Eclinic_portalModelImmunisation_vaccine_types extends JModelList
{
	...
	public function getExportData($pks, $user = null)
	{
		...
		if (($pks_size = Eclinic_portalHelper::checkArray($pks)) !== false || 'bulk' === $pks)
		{
			...
			// [Interpretation 14528] From the eclinic_portal_immunisation_vaccine_type table
			$query->from($db->quoteName('#__eclinic_portal_immunisation_vaccine_type', 'a'));
	...
	*/
	...
	protected function checkInNow()
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__eclinic_portal_immunisation_vaccine_type'));
			$db->setQuery($query);
			$db->execute();
			if ($db->getNumRows())
			{
				...
				$query->update($db->quoteName('#__eclinic_portal_immunisation_vaccine_type'))->set($fields)->where($conditions); 

 ...
 */
class Eclinic_portalModelFamily_plannings extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__eclinic_portal_family_planning', 'a'));

		// [Interpretation 15382] From the eclinic_portal_planning_type table.
		$query->select($db->quoteName('g.name','diagnosis_name'));
		$query->join('LEFT', $db->quoteName('#__eclinic_portal_planning_type', 'g') . ' ON (' . $db->quoteName('a.diagnosis') . ' = ' . $db->quoteName('g.id') . ')');
	...
	public function getExportData($pks, $user = null)
	{
		...
		if (($pks_size = Eclinic_portalHelper::checkArray($pks)) !== false || 'bulk' === $pks)
		{
			...
			// [Interpretation 14528] From the eclinic_portal_family_planning table
			$query->from($db->quoteName('#__eclinic_portal_family_planning', 'a'));
	...
	*/
	public function getExImPortHeaders()
	{
		...
		$db = JFactory::getDbo();
		// get the columns
		$columns = $db->getTableColumns("#__eclinic_portal_family_planning");
	...
	protected function checkInNow()
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__eclinic_portal_family_planning'));
			$db->setQuery($query);
			$db->execute();
			if ($db->getNumRows())
			{
				...
				$query->update($db->quoteName('#__eclinic_portal_family_planning'))->set($fields)->where($conditions); 

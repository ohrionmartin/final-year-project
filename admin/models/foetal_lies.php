 ...
 * Foetal_lies Model
 */
class Eclinic_portalModelFoetal_lies extends JModelList
{
	...
	public function getExportData($pks, $user = null)
	{
		...
		if (($pks_size = Eclinic_portalHelper::checkArray($pks)) !== false || 'bulk' === $pks)
		{
			...
			// [Interpretation 14528] From the eclinic_portal_foetal_lie table
			$query->from($db->quoteName('#__eclinic_portal_foetal_lie', 'a'));
	...
	*/
	...
	protected function checkInNow()
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__eclinic_portal_foetal_lie'));
			$db->setQuery($query);
			$db->execute();
			if ($db->getNumRows())
			{
				...
				$query->update($db->quoteName('#__eclinic_portal_foetal_lie'))->set($fields)->where($conditions); 

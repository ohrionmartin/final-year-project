...
/**
 * Eclinic_portal Medication Model
 */
class Eclinic_portalModelMedication extends JModelAdmin
{
	/**
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_eclinic_portal.medication';
	 ...
	 * @since   1.6
	 */
	public function getTable($type = 'medication', $prefix = 'Eclinic_portalTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_eclinic_portal.medication');
			}
		}
	...
	public function getForm($data = array(), $loadData = true, $options = array('control' => 'jform'))
	{
		...
		$form = $this->loadForm('com_eclinic_portal.medication', 'medication', $options, $clear, $xpath);
		...
		if ($id != 0 && (!$user->authorise('core.edit.state', 'com_eclinic_portal.medication.' . (int) $id))
		...
		if ($id != 0 && (!$user->authorise('medication.edit.name', 'com_eclinic_portal.medication.' . (int) $id))
			|| ($id == 0 && !$user->authorise('medication.edit.name', 'com_eclinic_portal')))
		{
		...
		if ($id != 0 && (!$user->authorise('medication.edit.description', 'com_eclinic_portal.medication.' . (int) $id))
			|| ($id == 0 && !$user->authorise('medication.edit.description', 'com_eclinic_portal')))
		{
			// [Interpretation 18819] Disable fields for display.
	...
	{
		return 'administrator/components/com_eclinic_portal/models/forms/medication.js';
	}
	...
	protected function canDelete($record)
	{
		if (!empty($record->id))
		{
			...
			return $user->authorise('core.delete', 'com_eclinic_portal.medication.' . (int) $record->id);
		}
		return false;
	}
	...
	protected function canEditState($record)
	{
		...
		if ($recordId)
		{
			...
			$permission = $user->authorise('core.edit.state', 'com_eclinic_portal.medication.' . (int) $recordId);
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		return JFactory::getUser()->authorise('core.edit', 'com_eclinic_portal.medication.'. ((int) isset($data[$key]) ? $data[$key] : 0)) or parent::allowEdit($data, $key);
	}
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					->select('MAX(ordering)')
					->from($db->quoteName('#__eclinic_portal_medication'));
				$db->setQuery($query);
				$max = $db->loadResult();
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_eclinic_portal.edit.medication.data', array());

		if (empty($data))
		{
			$data = $this->getItem();
			// run the perprocess of the data
			$this->preprocessData('com_eclinic_portal.medication', $data);
		}
	...
	public function batch($commands, $pks, $contexts)
	{
		...
		$this->canDo			= Eclinic_portalHelper::getActions('medication');
		$this->batchSet			= true;
	...
	public function save($data)
	{
		...
		if (in_array($input->get('task'), array('apply', 'save', 'save2new')) && (int) $input->get('id') == 0)
		{
			if ($data['alias'] == null || empty($data['alias']))
			{
				...
				$table = JTable::getInstance('medication', 'eclinic_portalTable');

				if ($table->load(array('alias' => $data['alias'])) && ($table->id != $data['id'] || $data['id'] == 0))
				{
					$msg = JText::_('COM_ECLINIC_PORTAL_MEDICATION_SAVE_WARNING');
				}

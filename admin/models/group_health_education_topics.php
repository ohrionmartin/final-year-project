...
/**
 * Group_health_education_topics Model
 */
class Eclinic_portalModelGroup_health_education_topics extends JModelList
{
	...
	public function getExportData($pks, $user = null)
	{
		...
		if (($pks_size = Eclinic_portalHelper::checkArray($pks)) !== false || 'bulk' === $pks)
		{
			...
			// [Interpretation 15018] From the eclinic_portal_group_health_education_topic table
			$query->from($db->quoteName('#__eclinic_portal_group_health_education_topic', 'a'));
	...
	*/
	...
	protected function checkInNow()
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__eclinic_portal_group_health_education_topic'));
			$db->setQuery($query);
			$db->execute();
			if ($db->getNumRows())
			{
				...
				$query->update($db->quoteName('#__eclinic_portal_group_health_education_topic'))->set($fields)->where($conditions); 

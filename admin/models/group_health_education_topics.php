 ...
 */
class Ehealth_portalModelGroup_health_education_topics extends JModelList
{
	...
	public function getExportData($pks, $user = null)
	{
		...
		if (($pks_size = Ehealth_portalHelper::checkArray($pks)) !== false || 'bulk' === $pks)
		{
			...
			// [Interpretation 14528] From the ehealth_portal_group_health_education_topic table
			$query->from($db->quoteName('#__ehealth_portal_group_health_education_topic', 'a'));
	...
	*/
	...
	protected function checkInNow()
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__ehealth_portal_group_health_education_topic'));
			$db->setQuery($query);
			$db->execute();
			if ($db->getNumRows())
			{
				...
				$query->update($db->quoteName('#__ehealth_portal_group_health_education_topic'))->set($fields)->where($conditions); 

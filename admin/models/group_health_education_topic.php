...
/**
 * Ehealth_portal Group_health_education_topic Model
 */
class Ehealth_portalModelGroup_health_education_topic extends JModelAdmin
{
	/**
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_ehealth_portal.group_health_education_topic';
	 ...
	 * @since   1.6
	 */
	public function getTable($type = 'group_health_education_topic', $prefix = 'Ehealth_portalTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_ehealth_portal.group_health_education_topic');
			}
		}
	...
	public function getForm($data = array(), $loadData = true, $options = array('control' => 'jform'))
	{
		...
		$form = $this->loadForm('com_ehealth_portal.group_health_education_topic', 'group_health_education_topic', $options, $clear, $xpath);
		...
		if ($id != 0 && (!$user->authorise('core.edit.state', 'com_ehealth_portal.group_health_education_topic.' . (int) $id))
		...
		if ($id != 0 && (!$user->authorise('group_health_education_topic.edit.name', 'com_ehealth_portal.group_health_education_topic.' . (int) $id))
			|| ($id == 0 && !$user->authorise('group_health_education_topic.edit.name', 'com_ehealth_portal')))
		{
		...
		if ($id != 0 && (!$user->authorise('group_health_education_topic.edit.description', 'com_ehealth_portal.group_health_education_topic.' . (int) $id))
			|| ($id == 0 && !$user->authorise('group_health_education_topic.edit.description', 'com_ehealth_portal')))
		{
			// [Interpretation 18819] Disable fields for display.
	...
	{
		return 'administrator/components/com_ehealth_portal/models/forms/group_health_education_topic.js';
	}
	...
	protected function canDelete($record)
	{
		if (!empty($record->id))
		{
			...
			return $user->authorise('core.delete', 'com_ehealth_portal.group_health_education_topic.' . (int) $record->id);
		}
		return false;
	}
	...
	protected function canEditState($record)
	{
		...
		if ($recordId)
		{
			...
			$permission = $user->authorise('core.edit.state', 'com_ehealth_portal.group_health_education_topic.' . (int) $recordId);
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		return JFactory::getUser()->authorise('core.edit', 'com_ehealth_portal.group_health_education_topic.'. ((int) isset($data[$key]) ? $data[$key] : 0)) or parent::allowEdit($data, $key);
	}
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					->select('MAX(ordering)')
					->from($db->quoteName('#__ehealth_portal_group_health_education_topic'));
				$db->setQuery($query);
				$max = $db->loadResult();
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_ehealth_portal.edit.group_health_education_topic.data', array());

		if (empty($data))
		{
			$data = $this->getItem();
			// run the perprocess of the data
			$this->preprocessData('com_ehealth_portal.group_health_education_topic', $data);
		}
	...
	public function batch($commands, $pks, $contexts)
	{
		...
		$this->canDo			= Ehealth_portalHelper::getActions('group_health_education_topic');
		$this->batchSet			= true;
	...
	public function save($data)
	{
		...
		if (in_array($input->get('task'), array('apply', 'save', 'save2new')) && (int) $input->get('id') == 0)
		{
			if ($data['alias'] == null || empty($data['alias']))
			{
				...
				$table = JTable::getInstance('group_health_education_topic', 'ehealth_portalTable');

				if ($table->load(array('alias' => $data['alias'])) && ($table->id != $data['id'] || $data['id'] == 0))
				{
					$msg = JText::_('COM_EHEALTH_PORTAL_GROUP_HEALTH_EDUCATION_TOPIC_SAVE_WARNING');
				}

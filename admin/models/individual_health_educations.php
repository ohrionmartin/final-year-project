 ...
 */
class Ehealth_portalModelIndividual_health_educations extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__ehealth_portal_individual_health_education', 'a'));

		// [Interpretation 15382] From the ehealth_portal_individual_health_education_topic table.
		$query->select($db->quoteName('g.name','individual_health_edu_name'));
		$query->join('LEFT', $db->quoteName('#__ehealth_portal_individual_health_education_topic', 'g') . ' ON (' . $db->quoteName('a.individual_health_edu') . ' = ' . $db->quoteName('g.id') . ')');
	...
	public function getExportData($pks, $user = null)
	{
		...
		if (($pks_size = Ehealth_portalHelper::checkArray($pks)) !== false || 'bulk' === $pks)
		{
			...
			// [Interpretation 14528] From the ehealth_portal_individual_health_education table
			$query->from($db->quoteName('#__ehealth_portal_individual_health_education', 'a'));
	...
	*/
	public function getExImPortHeaders()
	{
		...
		$db = JFactory::getDbo();
		// get the columns
		$columns = $db->getTableColumns("#__ehealth_portal_individual_health_education");
	...
	protected function checkInNow()
	{
		...
		if ($time)
		{
			...
			if ($db->getNumRows())
			{
				...
				$query->update($db->quoteName('#__ehealth_portal_individual_health_education'))->set($fields)->where($conditions); 

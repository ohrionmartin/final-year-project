 ...
 */
class Eclinic_portalModelCounseling_types extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__eclinic_portal_counseling_type', 'a'));
	...
	public function getExportData($pks, $user = null)
	{
		...
		if (($pks_size = Eclinic_portalHelper::checkArray($pks)) !== false || 'bulk' === $pks)
		{
			...
			// [Interpretation 15018] From the eclinic_portal_counseling_type table
			$query->from($db->quoteName('#__eclinic_portal_counseling_type', 'a'));
	...
	*/
	public function getExImPortHeaders()
	{
		...
		$db = JFactory::getDbo();
		// get the columns
		$columns = $db->getTableColumns("#__eclinic_portal_counseling_type");
	...
	protected function checkInNow()
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__eclinic_portal_counseling_type'));
			$db->setQuery($query);
			$db->execute();
			if ($db->getNumRows())
			{
				...
				$query->update($db->quoteName('#__eclinic_portal_counseling_type'))->set($fields)->where($conditions); 

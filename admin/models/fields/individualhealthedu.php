...
/**
 * Individualhealthedu Form Field class for the Ehealth_portal component
 */
class JFormFieldIndividualhealthedu extends JFormFieldList
{
	...
	protected function getInput()
	{
		...
		if ($set_button === 'true')
		{
			...
			// [Fields 5470] only add if user allowed to create individual_health_education_topic
			if ($user->authorise('core.create', 'com_ehealth_portal') && $app->isAdmin()) // TODO for now only in admin area.
			{
				...
				$button[] = '<a id="'.$button_code_name.'Create" class="btn btn-small btn-success hasTooltip" title="'.JText::sprintf('COM_EHEALTH_PORTAL_CREATE_NEW_S', $button_label).'" style="border-radius: 0px 4px 4px 0px; padding: 4px 4px 4px 7px;"
					href="index.php?option=com_ehealth_portal&amp;view=individual_health_education_topic&amp;layout=edit'.$ref.'" >
					<span class="icon-new icon-white"></span></a>';
			}
			// [Fields 5506] only add if user allowed to edit individual_health_education_topic
				...
				$script[] = "
					...
					function ".$button_code_name."Button(value) {
						if (value > 0) {
							...
							var url = 'index.php?option=com_ehealth_portal&view=individual_health_education_topics&task=individual_health_education_topic.edit&id='+value+'".$refJ."';
			...
			// [Fields 5581] check if button was created for individual_health_education_topic field.
	...
	protected function getOptions()
	{
		...
		$query->from($db->quoteName('#__ehealth_portal_individual_health_education_topic', 'a'));
		$query->where($db->quoteName('a.published') . ' = 1');
		$query->order('a.name ASC');
		// Implement View Level Access (if set in table)
		if (!$user->authorise('core.options', 'com_ehealth_portal'))
		{
			$columns = $db->getTableColumns('#__ehealth_portal_individual_health_education_topic');

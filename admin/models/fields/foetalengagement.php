...
/**
 * Foetalengagement Form Field class for the Eclinic_portal component
 */
class JFormFieldFoetalengagement extends JFormFieldList
{
	...
	protected function getInput()
	{
		...
		if ($set_button === 'true')
		{
			...
			// [Fields 5470] only add if user allowed to create foetal_engagement
			if ($user->authorise('core.create', 'com_eclinic_portal') && $app->isAdmin()) // TODO for now only in admin area.
			{
				...
				$button[] = '<a id="'.$button_code_name.'Create" class="btn btn-small btn-success hasTooltip" title="'.JText::sprintf('COM_ECLINIC_PORTAL_CREATE_NEW_S', $button_label).'" style="border-radius: 0px 4px 4px 0px; padding: 4px 4px 4px 7px;"
					href="index.php?option=com_eclinic_portal&amp;view=foetal_engagement&amp;layout=edit'.$ref.'" >
					<span class="icon-new icon-white"></span></a>';
			}
			// [Fields 5506] only add if user allowed to edit foetal_engagement
				...
				$script[] = "
					...
					function ".$button_code_name."Button(value) {
						if (value > 0) {
							...
							var url = 'index.php?option=com_eclinic_portal&view=foetal_engagements&task=foetal_engagement.edit&id='+value+'".$refJ."';
			...
			// [Fields 5581] check if button was created for foetal_engagement field.
	...
	protected function getOptions()
	{
		...
		$query->from($db->quoteName('#__eclinic_portal_foetal_engagement', 'a'));
		$query->where($db->quoteName('a.published') . ' = 1');
		$query->order('a.name ASC');
		// Implement View Level Access (if set in table)
		if (!$user->authorise('core.options', 'com_eclinic_portal'))
		{
			$columns = $db->getTableColumns('#__eclinic_portal_foetal_engagement');
